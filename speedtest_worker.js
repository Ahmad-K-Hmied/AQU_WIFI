...
var settings = {
	...
	telemetry_location: "", //extra data that can be passed to the telemetry through the settings
...
function sendTelemetry(done) {
	...
	try {
		...
		fd.append("location", settings.telemetry_location);
		xhr.send(fd);
	} catch (ex) {
		var postData = "location=" + encodeURIComponent(settings.telemetry_location) + "&ispinfo=" + encodeURIComponent(JSON.stringify(telemetryIspInfo)) + "&dl=" + encodeURIComponent(dlStatus) + "&ul=" + encodeURIComponent(ulStatus) + "&ping=" + encodeURIComponent(pingStatus) + "&jitter=" + encodeURIComponent(jitterStatus) + "&log=" + encodeURIComponent(settings.telemetry_level > 1 ? log : "");
