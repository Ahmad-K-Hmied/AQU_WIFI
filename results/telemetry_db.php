 ...
 */
function insertSpeedtestUser($ip, $ispinfo, $location, $ua, $lang, $dl, $ul, $ping, $jitter, $log, $returnExceptionOnError = false)
{
    $pdo = getPdo();
    ...
    try {
        ...
        (ip,ispinfo,location,ua,lang,dl,ul,ping,jitter,log)
        VALUES (?,?,?,?,?,?,?,?,?,?)'
        );
        $stmt->execute([
            $ip, $ispinfo, $location, $ua, $lang, $dl, $ul, $ping, $jitter, $log
...
function getSpeedtestUserById($id,$returnExceptionOnError = false)
{
    ...
    try {
        $stmt = $pdo->prepare(
            'SELECT
            id, timestamp, ip, ispinfo, ua, lang, dl, ul, ping, jitter, log, location
...
function getLatestSpeedtestUsers()
{
    ...
    try {
		...
		$sql .= ' id, timestamp, ip, ispinfo, ua, lang, dl, ul, ping, jitter, log, location
            FROM speedtest_users
